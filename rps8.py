...
#this updated version which uses command line arguments and personalizes the game using name as the argument.
...
def rps(name='playerOne'): # afunction which wraps the whole play_rps and acts as a parent function.
    ...
    def play_rps(): #ek function create kiya hai which is responsible for playing game.
        ...
        print("")
        print(f"{name}, you chose\t {str(RPS(player)).replace('RPS.','').title()}.") #we need to use playerchoice instead of player coz uska datatype string hai and concatenation happens between strings
        ...
        #compare who won
            if player == 1 and computer ==3:
                player_wins+=1
                return f"{name} you Win!🍾🍾" #we have replaced print with return to store the value of the result rather than just displaying it for the later use in decide_winner function
                ...
                python_wins+=1
                return f"Python Wins!🤡\nSorry, {name} 😜" #we have replaced print with return to store the value of the result rather than just displaying it for the later use in decide_winner function
        ...
        print(f"\n Game count:  {game_count}") #output to print how many times we have played the game. 
        print(f"\n {name}'s wins:  {player_wins}") #output to print how many times player won the game.
        print(f"\n Python wins:  {python_wins}") #output to print how many times python won the game.
...
#if rps7 is the file we are working on i.e. if this is the main file then call the function.
if __name__=="__main__":
    import argparse #argparse is a parser for command-line options, arguments and sub-commands.
    parser= argparse.ArgumentParser( #this is our parser
        description="provides a personal game experience." #description tells us the role of the parser.
    )
    parser.add_argument(
        "-n","--name",metavar="name", #
    ...
    rock_paper_scissors= rps(args.name) #rock_paper_scissors will store the return value of parent function rps().
    #we are passing our name argument to the rock_paper_scissors game.
    rock_paper_scissors()