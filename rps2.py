...
#this updated version contains loops.
...
class RPS(Enum):
    ...
    PAPER=2
    SCISSORS=3
#to create a condition where the system reboots itself to play the game rather than manually running the code each time.
playagain= True #if the condition is true the game will run again till playagain does not becomes false.
while playagain: #use while loop to run the game itself.
    ...
    if playagain.lower() == "y": #lowecase the input. if user enters capitals.
        continue #continue to play if user enters y, i.e. do not change the value of playagain.
    else:
        print("\n🥂🥂")
        print("Thankyou!\n")
        playagain=False #change the value of playagin to false if user enters n to exit the while loop.
        #can also use break instead of false.
sys.exit("Bye! 👋")